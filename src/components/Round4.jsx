...
import ctfimage from "../assets/gym-battle.png"; // Ensure the image path is correct
  ...
  const handleFlagSubmit = async (e) => {
    ...
    try {
      if (flag === "POKEMON123") {
        ...
        const documentData = {
          question: "What is the flag for Gym Battle Challenge?",
          answer: flag,
          points: points + 30,
        };
  ...
  return (
    <div className="relative flex flex-col items-center min-h-screen overflow-hidden text-white bg-gray-900">
      ...
      <div className="absolute inset-0 overflow-hidden">
        <div className="animate-bounce bg-blue-500 opacity-40 rounded-full w-[500px] h-[500px] blur-3xl absolute top-10 left-20"></div>
        <div className="animate-bounce bg-green-600 opacity-40 rounded-full w-[600px] h-[600px] blur-3xl absolute bottom-10 right-20"></div>
      </div>

      {/* Content */}
      <div className="container relative z-10 p-5 mx-auto">
        {/* Section Header with Text Animation */}
        <h1 className="mb-6 text-4xl font-bold text-center text-yellow-700 animate-pulse">
          Gym-Battle Challenge - Pokémon CTF
        </h1>

        {/* Question Text with Animation */}
        <p className="container mb-6 text-lg text-center text-white animate-pulse">
          Welcome to the Gym-Battle Challenge! Your task is to uncover the hidden flag
